...
sudo install -d -o deploy -g deploy -m 755 /var/www/cloudmountain/releases
sudo -u deploy install -d -m 755 /var/www/cloudmountain/releases/bootstrap
sudo rsync -a /path/to/current/site/ /var/www/cloudmountain/releases/bootstrap/
sudo chown -R deploy:deploy /var/www/cloudmountain
sudo -u deploy ln -s /var/www/cloudmountain/releases/bootstrap /var/www/cloudmountain/current
```

确认 `/var/www/cloudmountain/current/index.html` 存在。不要在没有确认当前网站目录的情况下运行复制命令。
...
```nginx
root /var/www/cloudmountain/current;
index index.html;
...
| 名称             | 内容                       |
| ---------------- | -------------------------- |
| `DEPLOY_PATH`    | `/var/www/cloudmountain`   |
...
```bash
ls -lt /var/www/cloudmountain/releases
readlink -f /var/www/cloudmountain/current
```

确认目标版本包含 `index.html`，再原子切换：

```bash
target=/var/www/cloudmountain/releases/RELEASE_ID
test -s "$target/index.html"
ln -s "$target" /var/www/cloudmountain/current.rollback
mv -Tf /var/www/cloudmountain/current.rollback /var/www/cloudmountain/current
```
